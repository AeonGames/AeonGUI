...
function(gitclone)
	...
	if(NOT IS_DIRECTORY "${ARG_PATH}")
		...
		execute_process(COMMAND ${GIT_EXECUTABLE} clone --depth 1 ${ARG_REPO} ${ARG_PATH} WORKING_DIRECTORY "${PROJECT_SOURCE_DIR}" RESULT_VARIABLE git_result OUTPUT_VARIABLE git_output ERROR_VARIABLE git_output)
...
function(download url filename)
	if(NOT EXISTS "${PROJECT_SOURCE_DIR}/${filename}")
		message(STATUS "Downloading ${url}")
		file(DOWNLOAD "${url}" "${PROJECT_SOURCE_DIR}/${filename}" STATUS download_status SHOW_PROGRESS)
		list(GET download_status 0 download_status_code)
		list(GET download_status 1 download_status_desc)
		if(download_status_code)
			file(REMOVE "${PROJECT_SOURCE_DIR}/${filename}")
...
function(decompress filename directory)
	if(NOT IS_DIRECTORY "${PROJECT_SOURCE_DIR}/${directory}")
		message(STATUS "Extracting ${filename} into ${PROJECT_SOURCE_DIR}")
		execute_process(COMMAND ${CMAKE_COMMAND} -E tar xzvf "${PROJECT_SOURCE_DIR}/${filename}" WORKING_DIRECTORY "${PROJECT_SOURCE_DIR}" RESULT_VARIABLE extract_result)
		if(extract_result)
...
function(decompress_into filename directory)
	if(NOT IS_DIRECTORY "${PROJECT_SOURCE_DIR}/${directory}")
		...
		message(STATUS "Extracting ${filename} into ${PROJECT_SOURCE_DIR}/${directory}")
		execute_process(COMMAND ${CMAKE_COMMAND} -E tar xzvf "${PROJECT_SOURCE_DIR}/${filename}" WORKING_DIRECTORY "${PROJECT_SOURCE_DIR}/${directory}" RESULT_VARIABLE extract_result)
		if(extract_result)
			message(FATAL_ERROR "Extracting Failed with error code: ${extract_result}")
		endif()
	endif()
